...
function addToDoItem(text) {
  if (typeof text !== 'string') {
    console.error('Invalid input: Text of a todo item must be a string.');
    return;
  }
  const newTodo = {
    id: nextTodoId,
...
function removeToDoItem(todoId) {
  if (typeof todoId !== 'number') {
    console.error('Invalid input: Input must be a number.');
    return;
  }
...
function markToDoItemAsCompleted(todoId) {
  ...
  const todoIndex = todoItems.findIndex(function (todo) {
    return todo.id === todoId;
  });
  if (todoIndex !== -1) {
